...
void process_request(Connection &self, Request &r) {
  ...
  std::string server_message = self.m_server->m_router.process_request(r);

  bytes_sent =
      write(self.m_socket, server_message.c_str(), server_message.size());

  if (u64(bytes_sent) == server_message.size()) {
    sys::info("------ Server Response sent to client ------");
...
void client_thread(Connection *self) {
  ...
  while (true) {
    ...
    if (bytes_received == 0) {
      ...
      sys::fatal_error("Failed to read bytes from client socket connection");
    }
