...
void generate_cmake_lists(Config const &c) {
  ...
  ofs << "file(GLOB_RECURSE SRCS ${CMAKE_SOURCE_DIR}/" << c.cpp.src_dir << "/*.cpp ${CMAKE_SOURCE_DIR}/" << c.cpp.src_dir << "/*.c)\n";
  ofs << "file(GLOB_RECURSE HEADERS ${CMAKE_SOURCE_DIR}/" << c.cpp.src_dir << "/*.hpp ${CMAKE_SOURCE_DIR}/" << c.cpp.src_dir << "/*.h)\n";
  ...
  ofs << "endif()\n";
  ofs << "target_include_directories(" << c.cpp.lib_name << " PRIVATE ${CMAKE_SOURCE_DIR}/.cppweb/include";
  ...
  if (!c.cpp.modules.empty()) ofs << " ${CMAKE_SOURCE_DIR}/.cppweb/lib";
...
void maybe_rebuild_cmakelists() {
  if (!std::filesystem::exists("CMakeLists.txt") || std::filesystem::last_write_time("cppweb.conf") > std::filesystem::last_write_time("CMakeLists.txt")) {
