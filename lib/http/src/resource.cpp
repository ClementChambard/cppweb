...
static void setup_resource_type(Resource &res) {
  ...
  if (res.file_name.ends_with(".svg") || res.file_name.ends_with(".svg.gz"))
    res.resource_type = "image/svg+xml";
...
static void
cache_results_of_res(Resource &res,
                     std::optional<sys::FInfo> old_info = std::nullopt) {
...
Resource const &Resource::get(std::string const &name, bool cache_results) {
  ...
  if (auto const &it = g_RESOURCE_CACHE.find(name);
      it != g_RESOURCE_CACHE.end()) {
    auto &res = it->second;
    ...
    if (cache_results || res.cache_contents)
      cache_results_of_res(res, old_info);
    return res;
  } else {
    Resource res(file_name);
    if (cache_results || res.cache_contents)
      cache_results_of_res(res);
