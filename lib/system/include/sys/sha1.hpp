...
private:
  ...
  static uint32_t make_word(const uint8_t *p) {
    return ((uint32_t)p[0] << 3 * 8) | ((uint32_t)p[1] << 2 * 8) |
           ((uint32_t)p[2] << 1 * 8) | ((uint32_t)p[3] << 0 * 8);
  }
...
public:
  ...
  const sha1 &print_hex(char *hex, bool zero_terminate = true,
...
inline std::string base_64_sha1(std::string input) {
  char base64[SHA1_BASE64_SIZE + 1];

  sha1(input.c_str()).finalize().print_base64(base64);
  base64[SHA1_BASE64_SIZE] = 0;
