...
struct LoggerConfig {
  ...
  LogLevel min_level_console = LogLevel::INFO;
...
void set_colors(::Config::logger_t::colors_t const &colors) {
  set_color(CONFIG.colors.lvl[int(LogLevel::EXTRA)].name,
            CONFIG.colors.lvl[int(LogLevel::EXTRA)].text, colors.extra);
  set_color(CONFIG.colors.lvl[int(LogLevel::INFO)].name,
            CONFIG.colors.lvl[int(LogLevel::INFO)].text, colors.info);
  set_color(CONFIG.colors.lvl[int(LogLevel::WARN)].name,
            CONFIG.colors.lvl[int(LogLevel::WARN)].text, colors.warn);
  set_color(CONFIG.colors.lvl[int(LogLevel::ERROR)].name,
            CONFIG.colors.lvl[int(LogLevel::ERROR)].text, colors.error);
  set_color(CONFIG.colors.lvl[int(LogLevel::FATAL)].name,
            CONFIG.colors.lvl[int(LogLevel::FATAL)].text, colors.fatal);
...
void set_config(const ::Config::logger_t &config) {
  ...
  if (config.min_level) {
    if (*config.min_level == "info")
      ...
      CONFIG.min_level = LogLevel::FATAL;
    else
      CONFIG.min_level = LogLevel::INFO;
  } else {
    CONFIG.min_level = LogLevel::INFO;
...
static char const *const level_strings[u64(LogLevel::LEVEL_COUNT)] = {
...
void log_to_file(const char *time, LogLevel lvl, const char *message) {
  std::ostringstream output;
...
void log_to_console(const char *time, LogLevel lvl, const char *message) {
...
void log_to_console_extra(const char *time, const char *extra,
  ...
  std::string extra_kind_color = CONFIG.colors.lvl[int(LogLevel::EXTRA)].name;
  ...
  oss << ch_color(CONFIG.colors.time) << time << ch_color(extra_kind_color)
      << "[" << extra
      << "]: " << ch_color(CONFIG.colors.lvl[int(LogLevel::EXTRA)].text)
...
void log_inner(LogLevel lvl, const char *extra, const char *message,
               va_list args) {
  bool to_file = CONFIG.log_to_file;
  bool to_console = true;
  if (lvl == LogLevel::EXTRA) {
...
#define LOG_INNER(lvl, message)                                                \
  ...
  log_inner(lvl, nullptr, message, args);                                      \
  va_end(args)

void info(char const *message, ...) { LOG_INNER(LogLevel::INFO, message); }

void warn(char const *message, ...) { LOG_INNER(LogLevel::WARN, message); }

void error(char const *message, ...) { LOG_INNER(LogLevel::ERROR, message); }

void fatal_error(char const *message, ...) {
  LOG_INNER(LogLevel::FATAL, message);
...
void log_extra(const char *extra, const char *message, ...) {
  va_list args;
  va_start(args, message);
  log_inner(LogLevel::EXTRA, extra, message, args);
  va_end(args);
}
